...
export async function GET(request: NextRequest) {
  ...
  // 特殊处理 emby 源（支持多源）
  if (sourceKey === 'emby' || sourceKey.startsWith('emby_')) {
    try {
      ...
      if (sourceKey.startsWith('emby_')) {
        embyKey = sourceKey.substring(5); // 'emby_'.length = 5
      }
      ...
      // 获取媒体详情
      const item = await client.getItem(query);
      ...
      if (item.Type === 'Movie') {
        ...
        const result = {
          source: sourceKey, // 保持与请求一致（emby 或 emby_key）
